...
| E8 | **Pins are static during the flyover showcase**: ThreeMarkers' pins, clusters, popups and tooltips don't currently hide or follow the flyover camera — they stay visible at their world-anchored positions even as the cinematic camera sweeps the scene. Likely fix: have flyover dispatch a "showcase active" signal that ThreeMarkers honours by hiding the entire overlay (or by switching to a flyover-aware camera reference). To be addressed in a separate PR. | 🟥 deferred for separate PR |
