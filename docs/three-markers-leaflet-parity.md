...
| E8 | **Pins are static during the flyover showcase**: ThreeMarkers' pins, clusters, popups and tooltips don't currently follow the flyover camera. PR #633 (showcase options modal) ships an interim hide-during-showcase using `ThreeMarkers.setVisible(false)` so they're not stuck floating where the schema view last left them. Follow-up PR will move pins onto a dedicated Three.js layer (e.g. `NCZ.LAYER_PINS`), expose them as a normal entry in the overlay-controls panel alongside roads/metro/buildings/districts so users can toggle 3D pin visibility independently of sidebar filters, and let the showcase modal's "Show pins" option flip the flyover camera's layer mask + swap ThreeMarkers' active camera reference to the perspective camera so positions project correctly. Layer-based design also leaves headroom for future per-layer features (pin LOD, category-tier filtering). | 🟥 deferred for separate PR |
...
| E10 | **Wave/shimmer artifact during the showcase**: a perceptible undulating effect appears on the terrain (and possibly the buildings) while the flyover camera is in motion. Hard to characterise from text alone — could be a relative of the sub-pixel triangle-aliasing pan-shimmer noted in PR #628's CHANGELOG entry (which was the leading suspect after the AF / edge-highlight / shadow-toggle eliminations didn't move the needle), or it could be tied specifically to the perspective camera + dynamic sun + shadow update cadence during waypoint interpolation. To investigate: try with shadows off, with sun position frozen, with audio off (rules out beat-driven theme cross-dissolve as the source), and with `logarithmicDepthBuffer` toggled. Likely lands as its own diagnostic + fix PR once the cause is narrowed. | 🟥 deferred for investigation |
