...
- **Pin clustering in 3D** — projects each visible pin to screen pixels, groups within `PIN_3D_CLUSTER_RADIUS_PX` (40 px, matches Leaflet's `maxClusterRadius`), renders cluster bubbles with the same `marker-cluster-step-N` colour ramp as Leaflet. Re-cluster runs on `controls.change` (rAF-debounced — at most one recompute per frame). Cluster centroids are world-space; CSS2DRenderer projects them automatically. Reuses the existing `.marker-cluster*` CSS so 2D and 3D bubbles look identical.
