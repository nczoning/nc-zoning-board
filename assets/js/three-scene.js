...
const ThreeScene = (() => {
  ...
  let _lightsFactor = 0; // emissive strength of the CITY LIGHTS (windows, signage, glow). == _nightFactor unless NCZ.LIGHTS_MODE overrides the sun.
  ...
  // the city goes dark, and the warning below says why. Silent darkness would read as a
  ...
  let _windowBoxes = null;
  async function loadWindowBoxes(assetSet) {
    // TWO NAMES FOR ONE THING, and it cost us the whole vanilla asset set.
    //
    // The renderer calls the base-game cloud `cdpr` (NCZ.ASSET_SET_KEY). The extractor and
    // the wiki call it `vanilla`. The bake's manifest said `vanilla`, the scene said `cdpr`,
    // the equality check below rejected a perfectly correct file, and the scene fell back to
    // the old per-district model while reporting success. Normalise once, here, and never
    // compare the raw strings again.
    const cloud = assetSet === 'fixed' ? 'fixed' : 'vanilla';
    try {
      const [man, bin] = await Promise.all([
        fetch(NCZ.WINDOW_BOXES_URL(cloud)).then((r) => (r.ok ? r.json() : Promise.reject(new Error(r.status)))),
        fetch(NCZ.WINDOW_BOXES_BIN_URL(cloud)).then((r) => (r.ok ? r.arrayBuffer() : Promise.reject(new Error(r.status)))),
      ]);
      ...
      const offsets = new Map(man.districts.map((d) => [d.name, d.offset]));
      _windowBoxes = {
        counts,
        // Dequantise on demand, per district: byte/255 × shareMax → the glass share the
        // shader's pane-size solve expects (same units as the old per-subdistrict value).
        share(name) {
          const off = offsets.get(name), n = counts.get(name);
          const out = new Float32Array(n);
          for (let i = 0; i < n; i++) out[i] = (bytes[off + i] / 255) * man.shareMax;
          return out;
        },
      };
      const lit = bytes.reduce((a, v) => a + (v ? 1 : 0), 0);
      console.log(`[NCZ] window bake: ${bytes.length.toLocaleString()} boxes, ${lit.toLocaleString()} with real glass (${(100 * lit / bytes.length).toFixed(1)}%)`);
    } catch (e) {
      console.warn(`[NCZ] window bake unavailable (${e.message}) — falling back to the per-subdistrict glass profile`);
      _windowBoxes = null;
    }
  }
  ...
  //
  // 9 floats per panel: position(3) quaternion(4) size(2), already in THREE space.
  //
  ...
  let _panelMesh = null;
  async function buildWindowPanels(group) {
    if (NCZ.WINDOW_MODEL !== 'panels') return;
    let man, bin;
    try {
      [man, bin] = await Promise.all([
        fetch(NCZ.WINDOW_PANELS_URL).then((r) => (r.ok ? r.json() : Promise.reject(new Error(r.status)))),
        fetch(NCZ.WINDOW_PANELS_BIN_URL).then((r) => (r.ok ? r.arrayBuffer() : Promise.reject(new Error(r.status)))),
      ]);
    } catch (e) {
      console.warn(`[NCZ] window panels unavailable (${e.message}) — run: node scripts/window_panels.js --bake`);
      return;
    }
    const f = new Float32Array(bin);
    const n = man.count;
    if (f.length !== n * man.stride) {
      console.warn(`[NCZ] window panels: manifest says ${n} x ${man.stride} but the binary holds ${f.length} floats — IGNORED`);
      return;
    }
    ...
    const mat = new THREE.MeshBasicNodeMaterial({
      transparent: true,           // the wall between windows must not paint over the city
      blending: THREE.AdditiveBlending,
      depthWrite: false,           // emissive glass writes no depth
      depthTest: !noDepth,         // ...but IS occluded by buildings in front of it
      side: THREE.DoubleSide,      // a panel's baked normal may face into its own building
      toneMapped: true,
    });

    const mesh = new THREE.InstancedMesh(geo, mat, n);
    mesh.name = 'windowPanels';
    mesh.frustumCulled = false;
    const dummy = new THREE.Object3D();
    const _q = new THREE.Quaternion();
    const _n = new THREE.Vector3();
    // Panel SIZE goes to the GPU as a storage buffer: the window grid must tile in METRES
    // (a 3 m room is 3 m on a 24 m panel and on a 1.7 m one), so the shader needs each
    // panel's real dimensions, not the quad's normalised UV.
    const sizeBuf = instancedArray(n, 'vec2');
    const seedBuf = instancedArray(n, 'vec2');
    const sizes = sizeBuf.value, seeds = seedBuf.value;
    for (let i = 0; i < n; i++) {
      const o = i * man.stride;
      ...
      dummy.scale.set(f[o + 7], f[o + 8], 1);
      dummy.updateMatrix();
      mesh.setMatrixAt(i, dummy.matrix);
      sizes[i * 2] = f[o + 7];
      sizes[i * 2 + 1] = f[o + 8];
      // Seed the lit/unlit draw from the panel's WORLD POSITION, so the same panel is always
      // the same panel and neighbouring copies of one kit mesh do not share a pattern.
      seeds[i * 2] = f[o] * 0.137;
      seeds[i * 2 + 1] = f[o + 2] * 0.173 + f[o + 1] * 0.041;
    }
    ...
    const seed = seedBuf.element(instanceIndex);
    const nH = size.x.div(float(NCZ.WINDOW_ROOM_W)).round().max(float(1));
    const nV = size.y.div(float(NCZ.WINDOW_ROOM_H)).round().max(float(1));
    const cells = vec2(uv().x.mul(nH), uv().y.mul(nV));
    const cell = vec2(floor(cells.x), floor(cells.y));
    const inH = fract(cells.x);
    const inV = fract(cells.y);
    // The pane within its room, at the game's measured window aspect: the glass spans most of
    // the room's width and about a third of its height, the rest is frame/mullion.
    const mH = float((1 - NCZ.WINDOW_PANE_W) / 2);
    const mV = float((1 - NCZ.WINDOW_PANE_H) / 2);
    const pane = step(mH, inH).mul(step(inH, mH.oneMinus()))
      .mul(step(mV, inV)).mul(step(inV, mV.oneMinus()));
    // AmountTurnOffAtNight = 0.5 on 97% of placed glass: half the windows that EXIST go dark.
    // This is the material fact, and it is the only place a lit fraction belongs.
    const lit = step(float(1 - NCZ.WINDOW_LIT_IN_GLASS), hash21(cell.add(seed)));

    const hTint = hash21(cell.add(seed).add(vec2(53.7, 91.3)));
    const hHue = hash21(cell.add(seed).add(vec2(17.3, 68.9)));
    ...
    const winColor = mix(_windowColorUniforms[0], tint, step(float(1 - NCZ.WINDOW_TINT_FRACTION), hTint));

    const a = pane.mul(lit).mul(_buildingNightFactor).mul(float(NCZ.WINDOW_INTENSITY));
    mat.colorNode = winColor.mul(a);
    mat.opacityNode = a;

    group.add(mesh);
    _panelMesh = mesh;
    console.log(`[NCZ] window panels: ${n.toLocaleString()} real facade panels drawn (route A — no box cloud involved)`);
  }

  async function loadNightProfile() {
    try {
      const p = await fetch(NCZ.NIGHT_PROFILE_URL).then((r) => (r.ok ? r.json() : null));
      if (!p?.subdistricts) throw new Error('no subdistricts in profile');
      NCZ.NIGHT_PROFILE = p;
      const n = Object.keys(p.subdistricts).length;
      console.log(`[NCZ] night profile: ${n} subdistricts, glass source '${NCZ.GLASS_SHARE_SOURCE}', lit-in-glass ${NCZ.WINDOW_LIT_IN_GLASS}`);
    } catch (e) {
      console.warn(`[NCZ] night profile FAILED to load (${NCZ.NIGHT_PROFILE_URL}) — every building falls back to glass ${NCZ.GLASS_SHARE_DEFAULT}, so the city will be dark. Run: node scripts/night_analyse.js --emit data/night-profile.json`, e);
    }
  }
  ...
  function buildDistrictMeta(cloudName, attr, bcx, bcy, bcz, count, polys) {
    ...
    const glassShareById  = new Map();  // buildingId → MEASURED glass share (night-profile.json)
    let untagged = 0;
    for (const [id, a] of agg) {
      ...
      const glass = NCZ.glassShareFor(tag.subId, tag.districtId);
      glassShareById.set(id, glass);
      if (!tag.subId && !tag.districtId) untagged++;
      _buildingMeta.push({
  ...
  async function loadBuildings() {
    ...
    try {
      ...
      // the polygons and immediately looks its glass share up in the profile.
      // Asset set is read HERE, before the loads, because the window bake is per-cloud and
      // has to fetch the matching one (see loadWindowBoxes). It is re-read below for the
      // texture paths; same key, same value, one source of truth.
      const _assetSet = (typeof localStorage !== 'undefined' && localStorage.getItem(NCZ.ASSET_SET_KEY)) || NCZ.ASSET_SET_DEFAULT;
      const [_districtPolys] = await Promise.all([loadDistrictPolys(), loadNightProfile(), loadWindowBoxes(_assetSet)]);
      ...
      for (const meta of DISTRICT_META) {
        ...
        const { signDensityById: _signDensityById, glassShareById: _glassShareById } =
          buildDistrictMeta(meta.name, buildingAttrData, bcx, bcy, bcz, validCount, _districtPolys);
        ...
        // Per-instance zone OVERRIDE + MEASURED GLASS SHARE, as one vec4:
        ...
        //
        // PER-SUBDISTRICT (?winmodel=district). The previous model: one measured scalar
        // for every building in a subdistrict. It cannot express "this slab is glazed, the
        // one beside it is solid", which is what the city actually looks like. Kept for
        // the A/B, because a claim you cannot switch off is not a claim.
        // THERE IS NO FALLBACK, AND THAT IS DELIBERATE.
        //
        // The per-subdistrict glass profile used to sit behind this as a safety net. It is
        ...
        //
        // If the bake is missing or its indices do not line up, the windows go DARK and the
        // console says why. Dark is a bug you can see. A plausible wrong city is not.
        const _boxShare = _windowBoxes && _windowBoxes.counts.get(meta.name) === validCount
          ? _windowBoxes.share(meta.name) : null;
        if (!_boxShare && !_glassOff) {
          const had = _windowBoxes ? _windowBoxes.counts.get(meta.name) : 'no bake loaded';
          console.error(`[NCZ] ${meta.name}: NO WINDOW DATA — bake has ${had} boxes, the district decoded ${validCount}. `
            + 'Windows will be DARK here. Re-run: node scripts/window_boxes.js --set '
            + `${assetSet === 'fixed' ? 'fixed' : 'vanilla'} --margin 2 --assign volume --bake`);
        }
        for (let i = 0; i < validCount; i++) {
          overrideData[i * 4 + 2] = _glassOff ? 0.2 : (_boxShare ? _boxShare[i] : 0);
        }
        for (const z of _overrideZones) {
          ...
          for (let i = 0; i < validCount; i++) {
            ...
            if (NCZ.pointInPolygon([bcx[i], -bcz[i]], z.footprint)) { overrideData[i * 4] = code; overrideData[i * 4 + 1] = param; }
          }
        }
        const buildingOverrideBuffer = instancedArray(validCount, 'vec4');
        buildingOverrideBuffer.value.set(overrideData);
      ...
      // ── ROUTE A: THE REAL FACADE PANELS ───────────────────────────────────
      // Windows drawn on the game's own glass-bearing panels, at their exact world
      // transforms, instead of painted onto our approximation of the buildings.
      // ?winmodel=panels (see buildWindowPanels).
      await buildWindowPanels(group);
  ...
  function buildBuildingMaterial(meta, mTex, instanceMatricesBuffer, visibleIndicesBuffer, buildingAttrBuffer, buildingOverrideBuffer, buildingCBSBuffer, faceOccIdxBuffer, faceInvBuffer, partIdBuffer) {
    ...
    const applyOverrides = (m) => {
      let r = mix(m, float(0), isExclude);
      r = mix(r, float(1), isForceLit);
      return mix(r, fcLit, isForceClass);
    };
    const winMask = applyOverrides(archMaskGeo);    // windows
    ...
    // ── THE WINDOW MODEL ──────────────────────────────────────────────────────
    // `pane` = is this fragment inside a window? `lit` = is that window on?
    ...
    if (NCZ.WINDOW_MODEL === 'legacy') {
      ...
      pane = step(float(marginH), inH).mul(step(inH, float(1 - marginH)))
      ...
      lit = step(litFrac.mul(fracScale).oneMinus(), cellRnd);
    } else {
      // ── THE MEASURED MODEL ─────────────────────────────────────────────────
      //
      ...
      //
      //     lit_area  =  paneArea  x  P(lit)
      //               =  glassShare(subdistrict)  x  WINDOW_LIT_IN_GLASS
      //
      // So the district's measured glass share sets the pane's AREA, and the game's
      // measured window aspect sets its SHAPE. Charter Hill (17.4%) gets fat windows;
      // Northside (0.7%, a genuinely windowless industrial dock) gets slivers and reads
      ...
      // right physics.
      const glassShare = mix(bOverride.z, bOverride.z.max(oParam), isForceLit)
        .mul(float(NCZ.GLASS_SHARE_GAIN))
        .clamp(float(0), float(0.9));

      // Split that AREA into a width and a height at the game's measured window aspect.
      //   A       = (paneW·cellW) / (paneH·cellH)   ← the window's real w:h, = 2.14
      ...
      // Centred rectangle in the cell; the surrounding band is dark wall.
      const mH = paneW.oneMinus().mul(0.5);
      const mV = paneH.oneMinus().mul(0.5);
      pane = step(mH, inH).mul(step(inH, mH.oneMinus()))
        .mul(step(mV, inV)).mul(step(inV, mV.oneMinus()));

      // Is that window ON? The game's own constant, and the ONE term with no district,
      // archetype or towerness dependence: window_parallax_interior.mt sets
      // AmountTurnOffAtNight, and 1 - it = 0.5536 across 86% of the city's glass area.
      // If this ever needs turning, the thing that is wrong is somewhere else.
      lit = step(float(NCZ.WINDOW_LIT_IN_GLASS).oneMinus(), cellRnd);
    }
    ...
    // Compose: colour × pane × lit × wall-mask × lightsFactor × intensity, with a mild
    // _m modulation so denser/taller blocks glow a touch more.
    //
    ...
    const winIntensityMul = NCZ.WINDOW_MODEL === 'legacy'
      ...
      : NCZ.WINDOW_MODEL === 'panels' ? float(0)
      : float(1);
    const windowEmissive = winColor
      .mul(pane).mul(lit).mul(shellGate).mul(faceGate).mul(onWall).mul(winMask)
      .mul(winIntensityMul)
    ...
    if (new URLSearchParams(location.search).has('glassdebug')) {
      const g = bOverride.z.div(float(0.30)).clamp(float(0), float(1));
      let dbg = mix(vec3(0.05, 0.10, 0.55), vec3(0.10, 0.80, 0.35), g.div(0.33).clamp(0, 1));
      dbg = mix(dbg, vec3(0.95, 0.85, 0.15), g.sub(0.33).div(0.33).clamp(0, 1));
      dbg = mix(dbg, vec3(0.95, 0.15, 0.10), g.sub(0.66).div(0.34).clamp(0, 1));
      mat.colorNode = vec3(0.0, 0.0, 0.0);
      mat.emissiveNode = dbg;
      return mat;
    }
    ...
    // measured 0.20, was hand-set to 1.0), has nothing to offer a model that knows where
    // every sign in Night City physically is.
    //
    // It comes out NOW rather than after, because signage is the loudest thing in the night
    // scene and the windows cannot be judged underneath it. The window A/B has to be a
    // window A/B.
    //
  ...
  function updateDayNightLighting() {
    ...
    const lf = _lightsFactor = NCZ.lightsFactor(nf);
    ...
    if (_glowIntensityU) _glowIntensityU.value = NCZ.LIGHTS_MODE === "off" ? 0
      : NCZ.CITY_GLOW_INTENSITY * (NCZ.CITY_GLOW_DAY_FACTOR + (1 - NCZ.CITY_GLOW_DAY_FACTOR) * lf);
  ...
  function setLightsMode(mode) {
    ...
    NCZ.LIGHTS_MODE = mode;
    updateDayNightLighting();   // recomputes lf, repaints (requestRender lives inside)
  }
  function getLightsMode() { return NCZ.LIGHTS_MODE; }
  // The emissive strength the lights are ACTUALLY running at, after the mode is applied.
  function getLightsFactor() { return _lightsFactor; }
