...
const ThreeScene = (() => {
  ...
  function buildDistrictMeta(cloudName, attr, bcx, bcy, bcz, count, polys) {
    ...
    if (untagged) console.warn(`[NCZ] ${cloudName}: ${untagged} buildings tagged to NO polygon → glass 0 (dark)`);
  ...
  async function loadBuildings() {
    ...
    try {
      ...
      const [_districtPolys] = await Promise.all([loadDistrictPolys(), loadNightProfile()]);
      ...
      for (const meta of DISTRICT_META) {
        ...
        for (let i = 0; i < validCount; i++) {
          overrideData[i * 4 + 2] = _glassOff ? 0.2
            : (_glassShareById.get(buildingAttrData[i * 4 + 3]) ?? NCZ.GLASS_SHARE_DEFAULT);
        }
