...
async function initMap() {
  ...
  // ?lighttune — live lighting-calibration panel. Sliders for sun/ambient/
  // exposure/shadow drive the ThreeScene setters directly. Hidden by default;
  // shown when the URL flag is set. Debug-only — not linked from any UI.
  if (new URLSearchParams(window.location.search).has('lighttune')) {
    const tuner = document.getElementById('lighting-tuner');
    if (tuner) {
      tuner.hidden = false;
      // Stop pointer/wheel events from bubbling to OrbitControls (otherwise
      // dragging a slider also pans the map — same problem the sun slider had).
      ['pointerdown', 'pointermove', 'pointerup', 'mousedown', 'mousemove', 'mouseup', 'wheel', 'touchstart', 'touchmove']
        .forEach(t => tuner.addEventListener(t, e => e.stopPropagation()));
      // Bind a slider + numeric input pair to a scene setter. Both inputs stay
      // in sync; either can drive. The number input is for precision/typed values
      // (sliders are coarse and snap to step).
      const bind = (id, defaultVal, setter, decimals = 3) => {
        const slider = document.getElementById(`lt-${id}`);
        const num    = document.getElementById(`lt-${id}-num`);
        if (!slider || !num) return;
        const apply = (v) => {
          v = Number.isFinite(v) ? v : defaultVal;
          slider.value = v;
        ...
        // Initial sync — slider/number show the default + the scene is driven.
        apply(defaultVal);
      };
      bind('sun',    NCZ.SUN_INTENSITY,     v => NCZ.ThreeScene?.setSunIntensity?.(v),     3);
      bind('amb',    NCZ.AMBIENT_INTENSITY, v => NCZ.ThreeScene?.setAmbientIntensity?.(v), 3);
      bind('exp',    NCZ.SCENE_EXPOSURE,    v => NCZ.ThreeScene?.setSceneExposure?.(v),    3);
      bind('shadow', NCZ.SHADOW_INTENSITY,  v => NCZ.ThreeScene?.setShadowIntensity?.(v),  3);

      document.getElementById('lt-pin-ref')?.addEventListener('click', () => {
        NCZ.ThreeScene?.setSunPosition?.(107.121956 * Math.PI / 180, 7 * Math.PI / 180);
      });
      // Helper — sync a slider/number pair to a value and drive the scene setter.
      const syncPair = (id, v, setter) => {
        const s = document.getElementById(`lt-${id}`); if (s) s.value = v;
        const n = document.getElementById(`lt-${id}-num`); if (n) n.value = (+v).toFixed(3);
        setter(+v);
      };
      document.getElementById('lt-reset')?.addEventListener('click', () => {
        syncPair('sun',    NCZ.SUN_INTENSITY,     v => NCZ.ThreeScene?.setSunIntensity?.(v));
        syncPair('amb',    NCZ.AMBIENT_INTENSITY, v => NCZ.ThreeScene?.setAmbientIntensity?.(v));
        syncPair('exp',    NCZ.SCENE_EXPOSURE,    v => NCZ.ThreeScene?.setSceneExposure?.(v));
        syncPair('shadow', NCZ.SHADOW_INTENSITY,  v => NCZ.ThreeScene?.setShadowIntensity?.(v));
      });
      document.getElementById('lt-copy')?.addEventListener('click', () => {
        // Read from the number inputs (the user's typed/slider values) — not the
        // live scene state, which can desync if other code paths touch the lights
        // (e.g. the Shadows overlay toggle resetting shadow intensity).
        const val = (id) => parseFloat(document.getElementById(`lt-${id}-num`).value);
        const snippet =
`NCZ.SUN_INTENSITY     = ${val('sun').toFixed(3)};
NCZ.AMBIENT_INTENSITY = ${val('amb').toFixed(3)};
NCZ.SCENE_EXPOSURE    = ${val('exp').toFixed(3)};
NCZ.SHADOW_INTENSITY  = ${val('shadow').toFixed(3)};`;
        navigator.clipboard.writeText(snippet).then(
          () => console.info('[NCZ] copied:\n' + snippet),
          err => console.warn('[NCZ] clipboard write failed:', err));
      });
      console.info('[NCZ] ?lighttune — lighting calibration panel active.');
    }
  }
