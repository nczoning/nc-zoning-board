...
function envFor({
  ...
  locationTags = [['loc-1', 'apartment']], checkedAt, nexusMissing,
} = {}) {
  ...
  return {
    ...
    DATASET: fakeKv(),
    DB: sqliteD1({ locations, locationTags, users: [user], nexusMissing }),
  };
}
...
test('every admin route is refused without a session', async () => {
  ...
  for (const [method, path] of [
    ...
    ['GET', '/admin/nexus-missing'], ['PATCH', '/admin/nexus-missing/12345'],
  ]) {
...
// owns the counter, and an admin who thinks the mod is gone for good edits the
// LOCATION, through the existing status control and the existing audit trail.

const MISSING_ROW = {
  nexus_id: '12345', miss_streak: 300, missing_since: '2026-07-01T00:00:00Z',
  last_missed_at: '2026-07-02T00:00:00Z', flagged_at: '2026-07-02T00:00:00Z',
};

test('the review list comes back with the pins that point at the missing mod', async () => {
  const env = envFor({ nexusMissing: [MISSING_ROW] });
  const res = await hit(env, 'GET', '/admin/nexus-missing');
  assert.equal(res.status, 200);
  const { missing } = await res.json();
  assert.equal(missing.length, 1);
  assert.equal(missing[0].nexus_id, '12345');
  assert.equal(missing[0].miss_streak, 300);
  assert.deepEqual(missing[0].locations, [{ id: 'loc-1', name: 'Existing Loft', status: 'published' }],
    'the pin is the thing the admin has to decide about');
});

test('dismissing a flag records who did it, and is audited', async () => {
  const env = envFor({ nexusMissing: [MISSING_ROW] });
  const res = await hit(env, 'PATCH', '/admin/nexus-missing/12345', { dismissed: true });
  assert.equal(res.status, 200);
  assert.equal((await res.json()).missing.dismissed_by, 'spuddeh');

  const row = env.DB.one('SELECT * FROM nexus_missing WHERE nexus_id = ?', '12345');
  assert.equal(row.dismissed_by, 'spuddeh');
  assert.ok(row.dismissed_at);
  assert.equal(row.miss_streak, 300, 'dismissing is not forgetting: the mod is still missing');

  const audit = auditRows(env);
  assert.equal(audit.length, 1);
  assert.equal(audit[0].action, 'nexus_missing.dismiss');
  assert.equal(audit[0].target, '12345');
});

test('dismissing never writes the location', async () => {
  const env = envFor({ nexusMissing: [MISSING_ROW] });
  await hit(env, 'PATCH', '/admin/nexus-missing/12345', { dismissed: true });
  assert.equal(env.DB.one('SELECT status FROM locations WHERE id = ?', 'loc-1').status, 'published',
    'no auto-hide, and no hide-by-side-effect either');
});

test('an untracked mod is 404, and a bad body is 422', async () => {
  const env = envFor({ nexusMissing: [MISSING_ROW] });
  assert.equal((await hit(env, 'PATCH', '/admin/nexus-missing/99999', { dismissed: true })).status, 404);
  assert.equal((await hit(env, 'PATCH', '/admin/nexus-missing/12345', { note: 'x' })).status, 422);
  assert.equal((await hit(env, 'DELETE', '/admin/nexus-missing/12345')).status, 405,
